 ...
 *
 * @module @se373/tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@se373/output-retention'
import type { RetainedItems } from '@se373/output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@se373/subprocess'
import type { SaveTextSpill, SpillRef } from '@se373/spill'
 ...
 * `@se373/tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`@se373/tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@se373/tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@se373/tool-fs-search/presentation} `grepSearchMeta`)
