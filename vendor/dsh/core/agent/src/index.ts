...
import { Context, FiberState, getTraceable, Service, symbols } from '@se373/cordis'
import type { Fiber } from '@se373/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@se373/scope'
import type { Scoped } from '@se373/scope'
import type { SessionEvent, SessionId } from '@se373/session'
import type { TypertContext, TypertLookup } from '@se373/typert-protocol'
 ...
 * (`@se373/agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@se373/agent#Agent',
