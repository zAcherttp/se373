 ...
 *
 * @module @se373/session
 */

import { Context, Service } from '@se373/cordis'
import { isAbsolute } from 'node:path'
import { deepFreeze } from '@se373/llm'
import { scopeOf, scopeTarget } from '@se373/scope'
import type { Scoped } from '@se373/scope'
import type { Message } from '@se373/llm'
import { SESSION_FORMAT_VERSION, SessionId } from './types.ts'
import type { TypertLookup } from '@se373/typert-protocol'
...
export type { AssistantMessage, ToolResultMessage, UserMessage } from '@se373/llm'
...
declare module '@se373/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@se373/scope`): agent-scoped listeners
     ...
     * Scope-filtered dispatch (`@se373/scope`) reuses the owner scope.
     ...
     * (`@se373/scope`) reuses the session's owner scope.
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('session', {
        parameter: 'session',
        wire: 'sessionId',
        hostTypeSymbol: '@se373/session#Session',
        wireTypeSymbol: '@se373/session/types#SessionId',
