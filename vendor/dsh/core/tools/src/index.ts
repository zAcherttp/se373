...
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@se373/scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@se373/scope'
import type { CallId, ContentBlock, ToolSchema } from '@se373/llm'
import { assertNever, deepFreeze, HarnessError } from '@se373/llm'
import type { Agent } from '@se373/agent'
import { snapshotJsonValue } from '@se373/session'
import type { JsonValue, UserMessage } from '@se373/session'
import type { ToolProviderResult } from '@se373/system-prompt'
import type { CodeRuntime } from '@se373/code-runtime'
...
export type { JsonValue } from '@se373/session'
...
// lives in its own UI-facing module; re-export it so `@se373/tools`
...
declare module '@se373/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@se373/scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@se373/scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@se373/scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@se373/tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@se373/tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`dsh-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @se373/code-runtime-worker-thread) or set tools mode to "native"`)
    }
