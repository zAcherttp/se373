 ...
 * @module @se373/tool-bash
 */

import type { Context } from '@se373/cordis'
import z from '@se373/schemastery'
import { isAbsolute, resolve as resolvePath } from 'node:path'
import { defineTool, TOOL_ABORTED } from '@se373/tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@se373/tools'
...
import type {} from '@se373/shell-env'
import type { SandboxExecutionPolicy, SandboxMode } from '@se373/sandbox'
import { ESCALATION_TARGETS, approveEscalation, canonicalPath, validateEscalationArgs } from '@se373/sandbox'
import type { SandboxPolicyService } from '@se373/sandbox-policy'
import { DSH_ENV_PREFIX } from '@se373/shell'
import type { ShellRunResult } from '@se373/shell'
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: BashToolArgs, exec) {
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @se373/jobs and @se373/tool-jobs')
        }
