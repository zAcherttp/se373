...
Sandbox-consuming Service Provider for the [`@se373/shell`](../shell/) executor seam. Load it **instead of** `@se373/bash-local`, together with a [`ctx.sandbox`](../../sandbox/sandbox/) provider (e.g. [`@se373/sandbox-local`](../../sandbox/sandbox-local/)) and a [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/) (which owns the default mode + workspace root, shared with the sandboxed filesystem) — no alternate tool plugin is needed; `dsh-tool-bash` detects the executor's `sandboxMode` capability and adds the escalation fields.
