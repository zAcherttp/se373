...
| Package | Role |
|---|---|
| `@se373/shell` (this) | Service Definition: abstract service + vocabulary types |
| `@se373/bash-local` | Service Provider: local subprocesses |
| `@se373/bash-sandbox` | Service Provider: `dsh-bash-local`'s mechanics with every spawn confined via [`ctx.sandbox`](../../sandbox/sandbox/), denials reported as result facts |
| `@se373/tool-bash` | the model-facing tool schemas over `ctx.shell` |
