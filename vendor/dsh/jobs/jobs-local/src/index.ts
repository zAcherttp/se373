 ...
 * @module @se373/jobs-local
 */

import { Context } from '@se373/cordis'
import z from '@se373/schemastery'
import type { Agent } from '@se373/agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@se373/scope'
import type { ScopeLayer } from '@se373/scope'
import { deadline, timeoutOf } from '@se373/timeout'
import { JobRegistry, JobId } from '@se373/jobs'
import type {
 ...
 * `@se373/jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @se373/tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @se373/agent)')
    }
