# @se373/skill-filesystem

English | [中文](README.zh.md)

Local filesystem provider for the `ctx.skills` registry.

This package implements one skill source. It scans local project, custom, and user skill roots, parses `SKILL.md` or flat Markdown skill files, and registers the provider on `ctx.skills`. The registry remains in `@se373/skill`; the durable session catalogs and model-facing loader tool remain in `@se373/tool-skill`.
...
| `dshHome` | `$DSH_HOME` or `~/.dsh` | DeepSeek Harness config root resolved by [`@se373/home-paths`](../../util/home-paths/README.md); scans `skills` under this directory. |
