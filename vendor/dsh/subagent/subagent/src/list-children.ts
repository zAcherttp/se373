...
import type { Session, SessionEvent, SessionHeader, SessionId } from '@se373/session'
import type { SessionPersistence } from '@se373/session-persistence'
import type { SessionProjectionRegistry } from '@se373/session-projection'
import type { SessionProjectionCache } from '@se373/session-projection-cache'
...
async function prepareListing(
  ...
  if (projections === undefined) {
    throw new SubagentError(
      'listing subagents requires the sessionProjections registry (load @se373/session-projection)',
  ...
  if (sessions === undefined) {
    throw new SubagentError(
      'listing subagents requires the session store (load @se373/session)',
