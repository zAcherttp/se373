...
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@se373/agent'
import type {} from '@se373/agent-presets/types'
import { AttachmentError, admitEncodedImages } from '@se373/attachment'
import type { ImageAttachmentRef } from '@se373/attachment'
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@se373/llm'
import { errorChain } from '@se373/llm'
import type { ContentBlock, MessageSource } from '@se373/llm'
import { isAppendSurfaceEvent, isJsonValue } from '@se373/session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@se373/session'
import type { SessionPersistence } from '@se373/session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@se373/session-query'
import { SubagentError } from '@se373/subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@se373/subagent'
import { isUserInvocable } from '@se373/skill'
import type { Workspace, WorkspaceRecord } from '@se373/workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@se373/workspace'
// Type-only: brings the `ctx.tools` Context merge into this program (viewFor reads presenters).
import {
  InvalidPresetIdError, PresetExistsError, PresetMountError,
  PresetNotWritableError, resolveSessionPreset, UnknownPresetError,
} from '@se373/agent-presets'
import type { PresetBearingSession } from '@se373/agent-presets'
import type {} from '@se373/tools'
import type {
...
import type { SessionRawArtifact } from '@se373/session-persistence'
import {
...
import type {} from '@se373/session-projection'
// Type-only: resolves `ctx.get('tasks')` to the background job registry.
import type {} from '@se373/jobs'
import type { JobSnapshot } from '@se373/jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@se373/session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@se373/goal'
import type { GoalRef as CoreGoalRef } from '@se373/goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@se373/commands'
...
import type {} from '@se373/skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@se373/settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@se373/settings'
import { credentialRef } from '@se373/credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@se373/session-title'
import type { CallId } from '@se373/llm/brand'
import type { ScopeKey } from '@se373/scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@se373/user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@se373/user-approval'
...
} from '@se373/user-questions'
import { UserQuestionError } from '@se373/user-questions'
import { DirectoryPickerError } from '@se373/host-directory-picker'
import {
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @se373/session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @se373/goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @se373/settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @se373/credentials-local) in its composition', details: {} }
  }
    ...
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @se373/session-query',
            details: {},
          })
        }
        try {
    ...
    skills: {
      ...
      async list(request) {
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @se373/skill', details: {} })
        }
