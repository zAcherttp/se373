...
| ~~D9~~ | ~~Push transport for the runtime graph~~ — **accepted 2026-08-29: poll-only for the semester.** Transitions are recorded as they happen and travel with the snapshot, so a polling board is a latency compromise, never a lossy one. Revisit only if §6.3's compare view (phase 8) needs live updates. | — | closed |
