...
The two solution files follow from the same source. `tsconfig.vendor.json` and
...
- **The client program does not compile yet.** `tsc -b tsconfig.vendor.json`
  (host) is clean; `tsc -b tsconfig.vendor.client.json` reports 143 errors, and
  the root of most of them is one gap: the **typert-generated `/remote`
  contributions**. Upstream generates `lib/typert.host.js` and
  `lib/typert.remote-client.js` per package during its build — `lib/` is
  gitignored there — via `typert/generator` and `typert/loader`, neither of
  which is in the `web-app` closure and neither of which is vendored. Until that
  codegen step exists, `@se373/commands/remote` and its siblings resolve to
  files that were never written. `tsdown` per client package and the Vite build
  sit behind it.
