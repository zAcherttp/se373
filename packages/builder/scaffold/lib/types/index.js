...
function checkName(name) {
    if (!SCAFFOLD_NAME.test(name)) {
        throw new ScaffoldError('SCAFFOLD_NAME', `scaffold name ${JSON.stringify(name)} must match ${String(SCAFFOLD_NAME)} — it becomes a directory name`);
    }
}
/**
 ...
 */
export function writeScaffold(root, name, tree) {
    checkName(name);
    const dir = join(resolve(root), name);
    if (existsSync(dir)) {
        throw new ScaffoldError('SCAFFOLD_EXISTS', `scaffold ${JSON.stringify(name)} already exists in ${root}; a scaffold never overwrites — pick a new name or remove it first`);
    }
 ...
 */
export function removeScaffold(root, name) {
    checkName(name);
