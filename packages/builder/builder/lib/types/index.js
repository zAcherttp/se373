 ...
 */
export function providedBy(block) {
    if (block === undefined)
        return [];
    if (block.manifest.provides !== undefined)
        return [...block.manifest.provides];
...
export class AgentBuilder extends Service {
    ...
    async plan(request) {
        ...
        const prefill = (recipe?.manifest.defaults ?? {});
        ...
        for (const row of rows) {
            for (const service of providedBy(this.ctx.blocks.get(row.id)))
                provided.add(service);
        }
        for (const row of rows) {
            const block = this.ctx.blocks.get(row.id);
            const missing = (block?.manifest.inject ?? []).filter(service => !provided.has(service) && this.ctx.get(service) === undefined);
            if (missing.length > 0) {
                warnings.push({
                    block: row.id,
        ...
        const plan = { planId, digest, spec, specRef: blockRef(registered), warnings };
     ...
     */
    async fabricate(digest) {
        const plan = this.plans.get(digest);
        if (plan === undefined)
            throw new Error(`no plan with digest ${digest.slice(0, 12)}…; call plan() first`);
        if (plan.planId !== null)
