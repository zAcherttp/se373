...
function recipe(id: string, summary: string, prefill: RecipePrefill, conformance?: string): BlockInput {
  return {
    id,
    kind: 'recipe',
    origin: 'system',
    ...
    manifest: {
      ...
      tier: 'ready',
      defaults: { ...prefill } as unknown as Record<string, unknown>,
    },
  }
}

/** Every shipped recipe, in the order SE373 lists the archetypes. */
export const COOKBOOK: readonly BlockInput[] = [
  recipe(
    'recipe.coding-agent',
    'An agent that reads, edits and runs code in a workspace',
    {
  ...
  recipe(
    ...
    'An agent that reviews a diff against the standards it can read',
    {
  ...
  recipe(
    'recipe.requirement-analysis',
    'An agent that turns a rough brief into checkable requirements',
    {
  ...
  recipe(
    'recipe.internal-knowledge',
    'An agent that answers from an indexed corpus rather than from memory',
    {
  ...
  recipe(
    ...
    'An agent that delegates parts of a task to subagents',
    {
  ...
  recipe(
    'recipe.mcp-assistant',
    'An agent whose tools come from an external MCP server',
    {
