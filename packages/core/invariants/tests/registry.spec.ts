...
describe('InvariantRegistry', () => {
  ...
  it('skips an installer excluded by the deny list but still reserves its name', async () => {
    const ctx = await mount({ deny: ['^@se373/noisy$'] })
    let ran = false
  ...
  it('admits only allow-listed packages when an allow list is set', async () => {
    const ctx = await mount({ allow: ['^@se373/kept$'] })
    const ran: string[] = []
  ...
  it('rejects invalid filter patterns at construction', async () => {
    await expect(mount({ deny: ['('] })).rejects.toThrow(/invalid regex/)
    await expect(mount({ allow: ['a', 'a'] })).rejects.toThrow(/duplicate regex/)
    await expect(mount({ allow: [' a'] })).rejects.toThrow(/non-blank/)
  })
