 ...
 */
export function assertComparable(generation, embedded, purpose) {
    if (embedded.fingerprint !== generation.fingerprint) {
        throw new Error(`refusing to ${purpose} generation ${generation.id}: it was written by ${generation.modelId} `
    ...
    if (embedded.dims !== generation.dims) {
        throw new Error(`refusing to ${purpose} generation ${generation.id}: it stores ${generation.dims}-dim vectors, `
