 ...
 */
export async function assertVectorStoreConformance(store) {
    ...
    try {
        ...
        if (hits.length !== 2)
            throw new Error(`asked for 2 hits and got ${hits.length}`);
        if (hits[0].key !== 'a') {
            throw new Error(`nearest-first is violated: expected "a" first, got ${JSON.stringify(hits[0].key)}`);
        }
        if (hits[0].text !== 'alpha' || hits[0].metadata?.n !== 1) {
        ...
        if (aHits.length !== 1)
            throw new Error(`key "a" appears ${aHits.length} times after upsert; upsert must replace`);
        if (aHits[0].text !== 'alpha-2')
            throw new Error('upsert must replace the stored text');
        const scanned = [];
        for await (const record of store.scan(generation.id))
            scanned.push(record.key);
        ...
        if (removed !== 1)
            throw new Error(`remove reported ${removed}, expected 1 (unknown keys are ignored, not errors)`);
        const afterRemove = await store.query(generation.id, embedded([0, 1, 0, 0]), 5);
        if (afterRemove.some(hit => hit.key === 'b'))
            throw new Error('a removed key still answers queries');
        const foreign = { fingerprint: '0'.repeat(64), dims: 4, vectors: [new Float32Array([1, 0, 0, 0])] };
