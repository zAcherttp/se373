...
import { modelDir } from "./cache.js";
/** Build the resolve URL for one file at a pinned revision. */
function sourceUrl(row, file) {
 ...
 */
async function fetchArtifact(row, dir, artifact, report, signal) {
    ...
    try {
        ...
        if (actual !== artifact.sha256) {
            throw new Error(`${artifact.file} hashed to ${actual}, pinned digest is ${artifact.sha256}. `
 ...
 */
export async function acquireRow(root, row, outstanding, options = {}) {
