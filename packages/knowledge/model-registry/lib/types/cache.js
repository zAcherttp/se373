 ...
 */
export function modelDir(root, row) {
 ...
 */
export async function resolveRow(root, row) {
    ...
    return {
        ...
        missing,
        bytes: outstanding,
        remedy: `model ${row.id} needs ${missing.length} file(s), ${megabytes} MB, from ${row.repo}@${row.revision.slice(0, 7)} `
