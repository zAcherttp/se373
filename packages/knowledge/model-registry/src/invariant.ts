...
function namedFiles(row: ModelRow): string[] {
  const { onnx, onnxData, tokenizer, tokenizerConfig } = row.files
  return [onnx, onnxData, tokenizer, tokenizerConfig].filter((file): file is string => file !== undefined)
}
