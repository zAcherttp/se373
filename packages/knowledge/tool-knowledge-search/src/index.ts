 ...
 */
export function renderHits(query: string, hits: readonly RetrievedChunk[]): string {
  if (hits.length === 0) return `No passages matched ${JSON.stringify(query)}.`
...
export function apply(ctx: Context): void {
  ...
  ctx.tools.register(defineTool({
    ...
    output: {
      ...
      render: (_args, value) => {
        const result = value as { query: string, hits: RetrievedChunk[] }
    ...
    async execute(args) {
      const hits = await ctx.knowledgePipeline.retrieve(
        args.query,
