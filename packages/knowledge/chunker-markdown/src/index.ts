...
export function sections(text: string): Section[] {
  ...
  const flush = (): void => {
    ...
    if (joined !== '' || heading !== null) out.push({ heading, text: joined })
    body = []
  }
...
export class MarkdownChunker extends Chunker {
  static override readonly name = 'chunker-markdown'
  ...
  chunk(document: Document): Chunk[] {
    ...
    for (const section of sections(document.text)) {
      const carried: Section = pending === null
        ? section
        : {
      ...
      if (carried.text.length < this.minSize) pending = carried
      else { merged.push(carried); pending = null }
    }
    ...
    for (const section of merged) {
      ...
      for (const span of spans) {
        ...
        chunks.push(buildChunk(
          document,
          chunks.length,
          section.heading === null ? span : `${section.heading}\n\n${span}`,
