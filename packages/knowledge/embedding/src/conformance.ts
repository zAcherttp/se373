...
import { fingerprintIdentity } from './fingerprint.ts'
import { templateFault } from './template.ts'
import { EMBED_ROLES, type EmbedderIdentity } from './types.ts'
// Type-only, so the index -> conformance -> index cycle is erased at runtime.
 ...
 */
export function describeIdentityFault(identity: EmbedderIdentity): string | null {
  ...
  if (identity.artifacts.length === 0) return 'declares no artifacts'
  for (const artifact of identity.artifacts) {
    if (!SHA256.test(artifact.sha256)) return `artifact ${artifact.file} has a malformed sha256`
  ...
  for (const role of EMBED_ROLES) {
    ...
    if (fault !== null) return `${role} template ${fault}`
  }
 ...
 */
export async function assertEmbedderConformance(embedder: Embedder): Promise<void> {
  const { identity } = embedder
  const fault = describeIdentityFault(identity)
  if (fault !== null) throw new Error(`identity: ${fault}`)
  ...
  for (const [index, vector] of first.vectors.entries()) {
    ...
    if (!vector.every(Number.isFinite)) throw new Error(`vector ${index} contains a non-finite component`)
