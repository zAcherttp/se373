 ...
 */
export function buildFeeds(session, batch) {
    ...
    for (const name of session.inputNames) {
        switch (name) {
            ...
            case 'position_ids':
                ...
                break;
            default:
                throw new Error(`graph requires an input this provider does not know how to build: ${name} `
 ...
 */
export function meanPool(hidden, dims, mask) {
    const [batchSize, sequence, width] = dims;
    const out = [];
 ...
 */
export function chooseOutput(session) {
    if (session.outputNames.includes('sentence_embedding'))
        return { name: 'sentence_embedding', pooled: true };
    if (session.outputNames.includes('last_hidden_state'))
        return { name: 'last_hidden_state', pooled: false };
    throw new Error(`graph exposes no usable embedding output (has: ${session.outputNames.join(', ')}); `
 ...
 */
export async function runBatch(session, identity, batch) {
    ...
    if (tensor === undefined)
        throw new Error(`graph did not return ${output.name}`);
    const data = tensor.data;
    const vectors = output.pooled
        ? Array.from({ length: tensor.dims[0] }, (_, b) => data.slice(b * tensor.dims[1], (b + 1) * tensor.dims[1]))
        : meanPool(data, tensor.dims, batch.attentionMask);
    ...
    return vectors.map((vector) => {
        ...
        if (identity.dims < identity.nativeDims)
            return truncateToDims(vector, identity.dims);
