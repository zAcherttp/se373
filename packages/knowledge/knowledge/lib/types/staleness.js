 ...
 */
export function generationKey(refs) {
 ...
 */
export function firstDivergence(stored, live) {
    for (const stage of WRITE_PATH_STAGES) {
        if (stored[stage] !== live[stage])
            return stage;
    }
    return null;
}
 ...
 */
export function rebuildPlan(from) {
    const at = WRITE_PATH_STAGES.indexOf(from);
    const needs = (stage) => WRITE_PATH_STAGES.indexOf(stage) >= at;
    return {
        from,
