...
import { GenerationDatabase } from './database.ts'
...
/** The one fact a flip changes. */
interface Manifest {
  active: string | null
}

/** Filename of a generation. */
function generationFile(dir: string, id: string): string {
...
export class SqliteVecStore extends VectorStore {
  static override readonly name = 'vs-sqlite-vec'
  ...
  private manifest(): Manifest {
    if (!existsSync(this.manifestPath)) return { active: null }
    try {
  ...
  private handle(id: string): GenerationDatabase {
  ...
  async active(): Promise<Generation | null> {
    const { active } = this.manifest()
    if (active === null) return null
    if (!existsSync(generationFile(this.dir, active))) return null
  ...
  async drop(id: string): Promise<void> {
    ...
    if (this.manifest().active === id) this.writeManifest({ active: null })
  }
   ...
   */
  async upsert(id: string, records: readonly VectorRecord[], embedded: EmbedResult): Promise<void> {
   ...
   */
  async query(id: string, embedded: EmbedResult, k: number): Promise<Hit[]> {
    const db = this.handle(id)
