...
const CANDIDATES: readonly Hit[] = Array.from({ length: 8 }, (_, index) => ({
 ...
 */
export async function assertRerankerConformance(reranker: Reranker): Promise<void> {
  ...
  if (out.length > k) throw new Error(`asked for ${k} and got ${out.length}; the caller sized a context budget on k`)
  ...
  for (const hit of out) {
    ...
    if (seen.has(hit.key)) throw new Error(`hit ${JSON.stringify(hit.key)} returned twice`)
    seen.add(hit.key)
  ...
  if (empty.length !== 0) throw new Error('an empty candidate list must rerank to an empty answer')
}
