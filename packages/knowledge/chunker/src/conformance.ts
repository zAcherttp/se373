...
const FIXTURES: readonly Document[] = [
  document('prose.md', Array.from({ length: 30 }, (_, i) =>
    `Sentence ${i} of an ordinary paragraph, with enough words to matter.`).join(' ')),
 ...
 */
export function assertChunkerConformance(chunker: Chunker): void {
  ...
  for (const fixture of FIXTURES) {
    ...
    for (const token of tokens(fixture.text)) {
      ...
      if (token.length > 48) {
        ...
        for (const char of token) need.set(char, (need.get(char) ?? 0) + 1)
