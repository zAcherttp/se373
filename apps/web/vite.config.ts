...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__DSH_BOOT__, '
  + 'which is the entry graph naming every client plugin bundle. The host is what injects it — boot a tree whose '
  + 'config carries the web rows (see examples/web/cordis.yml) and open the URL it prints.'
const DEFAULT_CLIENT_TITLE = 'SE373 Agentic Builder'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.SE373_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'se373-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>SE373 Agentic Builder</title>', `<title>${title}</title>`)
    },
  }
}
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'se373-reject-standalone-web-serve',
    config(_config, env) {
